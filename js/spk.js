...
import {
  ...
  sertakanLog, tandaBaru, nomorBerikutnya, nomorBerikutnyaBulanan,
} from "./db.js?v=3.16.0";
import { sesi, bolehAkses, konfirmasiPassword } from "./auth.js?v=3.16.0";
import { batasDiskon, muatBatasDiskon, PERAN } from "./roles.js?v=3.16.0";
import { DP_MINIMUM } from "./config.js?v=3.16.0";
import { muatTipe, tipeDari } from "./tipe.js?v=3.16.0";
import { cariUnitReady, muatSemuaUnitReadyRingkas,
  kunciUnitTransaksi, lepasUnitTransaksi } from "./stok.js?v=3.16.0";
import { formPelanggan, bacaFormPelanggan, simpanPelangganOtomatis,
         pasangHurufBesarPelanggan, cekTelepon } from "./pelanggan.js?v=3.16.0";
import { muatSaranKecamatan, muatSaranKota } from "./referensi.js?v=3.16.0";
import { muatLeasing, leasingAktif, leasingDari } from "./leasing.js?v=3.16.0";
import { muatRekening, rekeningAktif, rekeningDari } from "./rekening.js?v=3.16.0";
import { muatAgen, agenAktif } from "./agen.js?v=3.16.0";
import { cetakSpk, mintaCetakKuitansi as catatPembayaran, labelTombolKuitansi,
  ...
  cetakKoreksiRiwayatBayar, sudahLunas } from "./cetak.js?v=3.16.0";
import { konfirmasi, tanya, beritahu } from "./dialog.js?v=3.16.0";
import { buatNotifikasi, beriTahuSemuaOwner } from "./notifikasi.js?v=3.16.0";
import { rupiah, aman, kabar, pasangFormatUang, bacaAngka, namaTampilan, tanggal } from "./ui.js?v=3.16.0";
...
export async function halamanSpk(wadah) {
    ...
    return `<div style="overflow-x:auto;margin:8px 0">
      <table class="tabel">
        <thead><tr>
          ...
          <th>Tahun</th><th>Lokasi</th>${hargaBeda ? "<th>Harga OTR</th>" : ""}
        </tr></thead>
        <tbody>
  ...
  // Begitu isi Tunai (saat mode Tunai+Transfer sekaligus), kotak
  // Transfer OTOMATIS terisi sisanya menuju Harga Efektif (OTR −
  // Diskon) — asumsi paling umum: dua metode ini dipakai bareng buat
  // MELUNASI penuh di awal. Tetap bisa diedit manual kalau ternyata
  // bukan pelunasan penuh (mis. cuma DP gabungan dua metode).
  function perbaruiTransferOtomatis() {
    if (wadahTT.hidden) return; // cuma relevan kalau dua-duanya dicentang
    const diskonSaatIni = bacaAngka(diskonEl);
    const targetEfektif = Math.max(hargaOtrTerpilih - diskonSaatIni, 0);
    const tunai = bacaAngka(jmlTunaiEl);
    jmlTransferEl.value = Math.max(targetEfektif - tunai, 0).toLocaleString("id-ID");
  }
  ...
  function perbaruiCaraBayar() {
    ...
    perbaruiTagihanLeasing();
    perbaruiTransferOtomatis(); // langsung isi begitu mode Tunai+Transfer diaktifkan
  }
  ...
  // Cuma kotak TUNAI yang memicu isi-otomatis Transfer (bukan
  // sebaliknya) — supaya tidak saling menimpa terus-menerus kalau
  // dua-duanya sama-sama "auto-fill" satu sama lain.
  jmlTunaiEl.addEventListener("input", perbaruiTransferOtomatis);
  diskonEl.addEventListener("input", perbaruiTransferOtomatis);
  ...
  wadah.querySelector("#form-spk").addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // Nomor SPK bulanan: SPK/2026/IX/0001 — reset tiap ganti bulan.
      const spkNo = await nomorBerikutnyaBulanan("spk", "SPK");
